...
| **App icon and splash are untouched Expo template placeholders** | `apps/mobile/assets/` (`icon.png`, `splash-icon.png`, the three `android-icon-*`, `favicon.png`) | Never tracked here before 2026-09-19. `icon.png` still has its construction guides baked in — concentric circles, dashed axes, a centre crosshair — and `splash-icon.png` is blank graph paper. Both are light-themed, contradicting the OLED-black identity. Every user sees these on first launch. Design approved in `docs/superpowers/specs/2026-09-19-shellbell-brand-design.md`; assets not yet produced | 13 |
